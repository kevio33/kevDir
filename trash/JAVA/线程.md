...
2. 锁的获取方式不同：synchronized 在获取锁时是隐式的，不需要手动获取和释放锁。而 Lock 在获取锁时是显式的，需要通过 lock() 方法来获取锁，并通过 unlock() 方法来释放锁。
3. 锁的释放方式不同：synchronized 在离开同步代码块或方法时会自动释放锁，而 Lock 则需要手动调用 unlock() 方法来释放锁。
