...
- 字符流：每次读取(写出)两个字节，有中文时，使用该流就可以正确传输显示中文。
  ...
  - 节点流：以从或向一个特定的地方（节点）读写数据。即节点流是直接操作文件，网络等的流，例如FileInputStream和FileOutputStream，他们**直接从文件中读取或往文件中写入字节流。**
...
| 字节流                                                       | 字符流                                                       |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 没有缓冲区，是直接输出的。不调用colse()方法时，信息已经输出了 | 输出到缓冲区，只有在调用close()方法关闭缓冲区时，信息才输出。要想字符流在未关闭时输出信息，则需要手动调用flush()方法。 |
| 以字节（8bit）为单位                                         | 以字符为单位，根据码表映射字符，一次可能读多个字节。         |
