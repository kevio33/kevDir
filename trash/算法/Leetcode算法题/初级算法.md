...
> ```java
> public int findShortestSubArray(int[] nums) {
>     Map<Integer, int[]> map = new HashMap<>();// 保存两个相同数之间距离
>     Map<Integer, Integer> mapFreq = new HashMap<>();// 保存数字出现频率
>     int maxTime = 1;// 记录最大次数
>     for (int i = 0; i < nums.length; i++) {
>         if (map.containsKey(nums[i])) {
>             int[] distance = map.get(nums[i]);
>             distance[1] = i;
>             int freqTemp = mapFreq.get(nums[i]) + 1;
> 
>             maxTime = maxTime < freqTemp ? freqTemp : maxTime;// 出现频率最大的次数
>             mapFreq.replace(nums[i], freqTemp);// 更新出现频率
>         } else {
...
>         return 1;
> 
>     int minDistance = Integer.MAX_VALUE;// 记录子串长度
>     for (Map.Entry<Integer, Integer> entry : mapFreq.entrySet()) {
>         if (entry.getValue() == maxTime) {
>             int[] dis = map.get(entry.getKey());
>             if (minDistance > (dis[1] - dis[0]) && dis[1] - dis[0] > 0)
>                 minDistance = (dis[1] - dis[0]);
>         }
>     }
...
①先遍历存储所有字符出现次数；
②第二次再**按照字符串的顺序遍历**找出最小索引的那一个

其实不用担心顺序问题，因为最后按照字符串的熟悉进行遍历，就只需要存储字符出现的次数就够了。
...
> `que2`为辅助队列
>
> 每次插入数据时，先放到que2(栈顶)，然后将`que1`队列的数据取出放到`que2`，最后交换`que1`和`que2`
>
>  ![fig1](初级算法.assets/225_fig1.gif) 
>
> ```java
> class MyStack {
> 
...
>         queue2.offer(x);
>         while (!queue1.isEmpty()) {// 将que1元素出队列
...
> > 时间复杂度：
> >
> > 时间复杂度：入栈操作 O(n)，其余操作都是 O(1)，其中 n是栈内的元素个数。
> > 入栈操作需要将 queue1中的 n个元素出队，并入队 n+1 个元素到 queue2，共有 2n+1次操作，每次出队和入队操作的时间复杂度都是 O(1)，因此入栈操作的时间复杂度是 O(n)。

> **一个队列模拟栈**
>
> 每次插入元素先入队列，然后将该入队列前的元素依次出队列，再入队列，这样就将刚入队列的元素顶到队头，也就是栈顶了。
>
>  ![fig2](初级算法.assets/225_fig2.gif) 
>
> ```java
> class MyStack {
>     Queue<Integer> queue;
