...
基本思想：在待排序的n个记录中任取一个记录（一般取第一个记录），将该记录放到一个位置，原数据序列被划分成两部分，左边部分是比该记录小的数字，右边是比该序列大的数字，然后把该序列放到两部分的中间，这个过程称为一趟快速排序。之后分别对两个子序列递归进行快速排序
...
是一种非比较排序，再不限制内存情况下,对一群无序序列进行逐位比较，有下标为0-9的10个桶，先对个位进行比较，放入对应的桶中，然后在对十位比较，放入对应桶中，最大的数有几位就进行几次循环比较。最后得到排序结果;

![1617707852910](%E6%89%80%E6%9C%89%E6%8E%92%E5%BA%8F%E7%AE%97%E6%B3%95.assets/1617707852910.png)

利用二维数组来对每次比较的结果进行临时存储，数组的第一个下标表示对应的位上的数字，第二个数字对应在该位上的位置。
...
<img src="%E6%89%80%E6%9C%89%E6%8E%92%E5%BA%8F%E7%AE%97%E6%B3%95.assets/1617620617647.png" alt="1617620617647" style="zoom: 150%;" />
...
![1617777822565](%E6%89%80%E6%9C%89%E6%8E%92%E5%BA%8F%E7%AE%97%E6%B3%95.assets/1617777822565.png)
...
堆排序是一种选择排序。以大跟堆为例：此时整个序列最最大值就是根节点，然后交换根节点与末尾元素，这样末尾就变为了最大值，然后将n-1个元素重新构造成一个堆，重复n-1次，最后得到一个有序序列（一般升序采用大根堆，降序采用小根堆）
...
//调整为大根堆
void adjustHeap(int a[],int j,int length){
    int temp = a[j];//取出该元素
    int k = j;
    for(int i = j*2+1;i<length;i = i * 2 + 1){
        
        if(i+1<length && a[i]<a[i+1]){//先判断是否存在右子结点；如果右子节点大于左子结点，则指向右子节点
            i++;
        }

        if(temp < a[i]){//如果子节点大于父节点，将子节点值赋给父节点（不用进行交换）
            a[k] = a[i];
            k = i;//记录该结点位置
        }else{
            break;
    ...
    a[k] = temp;//将temp值放到最终的位置
}

void heapSort(int a[],int length){
    //排序前先调整为大跟堆,从最后一个非叶子结点开始，从下往上，从右往左
    //叶子结点不用调整
