...
一个都没找到的话，程序就会出错，会抛出`ActivityNotFoundException`。比如随便写一个action字符串：
...
intent中可以携带`bundle`对象，`bundle`对象携带要传输的数据
...
#### 1.概述：
 ...
 ![内容提供程序、其他类和存储空间之间的交互。](Android--四大组件.assets/content-provider-interaction.png) 

 `content provider`可以帮助应用管理对自身存储或由其他应用存储的数据的访问，并提供与其他应用共享数据的方法 
...
> > - `path`则是用于对同一应用程序中不同的表做区分，通常都会添加到`authority`后面：
>
> ```java
...
> > **防SQL注入**
> >
> > 推荐使用`selection`、`selectArgs`来定义查询的范围条件，例如：
> >
> > ```java
...
- 标准广播（无序广播）
  ...
  > 是一种完全**异步执行的广播**，广播发出后，所有的广播接收器几乎会在同一时刻接收到这条广播消息。但是意味着他无法被截断
  >
...
>  **注册方式：**
>
>  在`AndroidManifest.xml`里通过`<receive>`标签声明
>
>  **创建方式：**
>
>  右击创建`BroadcastReceiver`可以快速创建一个广播接收器
>
