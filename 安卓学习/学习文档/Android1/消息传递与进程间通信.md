...
>   需要注意的是，如果需要取消一个已经添加到消息队列中的Runnable对象，可以调用Handler的removeCallbacks()方法，传入需要取消的Runnable对象。例如： 
>
> ```java
...
由于应用程序之间不能共享内存。在不同应用程序之间交互数据（跨进程通讯），在Android SDK中提供了几种用于跨进程通讯的方式。
