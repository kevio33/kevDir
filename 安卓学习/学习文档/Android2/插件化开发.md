...
- 早期`Art`加载： 在**安装的时候**，会将dex文件直接编译成`.oat`这样的机器码，但这样会出现问题是：
...
> **只有在宿主包中的Manifest文件中注册的四大组件才能够启动，如果没有注册，就会抛出异常，提醒你在Manifest中注册**
...
那如果想要保存插件的动态化加载呢？也就是说想要在插件包中的Manifest文件中进行注册。
