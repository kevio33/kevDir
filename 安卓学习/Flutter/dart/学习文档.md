...
> - `Object`：是Dart 对象的基类，当你定义：` object o =xxx;`时这个时候系统会认为o是个对象，但是如果尝试调用`o.foo()`时，静态类型检查会运行报错。**综上不难看出dynamic 与object 的最大的区别是在静态类型检查上**。
...
Dart的类型安全意味着您不能使用`if（nonbooleanValue)`或`assert（nonbooleanValue）`等代码。
...
当对象不为空时正常调用相应的方法，当对象为空时则什么都不做，与kotlin一致
